var kafka = require('kafka-node'),
    ...
    createConsumerInstance = function (consumer, topic) {
        ...
        consumer.instance.on('message', function (m) {
            logger.trace(m, 'lib/consumerManager : consumer message received');
        ...
        consumer.instance.on('rebalancing', function () {
            logger.debug({consumer : consumer.id},'lib/consumerManager : rebalancing consumer');
        });

        consumer.instance.on('rebalanced', function () {
            logger.debug({consumer : consumer.id, topicsPayloads: consumer.instance.topicPayloads},'lib/consumerManager : rebalanced consumer');
        });
    },
